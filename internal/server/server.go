package server
import ("encoding/json";"log";"net/http";"github.com/stockyard-dev/stockyard-ponyexpress/internal/store")
...
func(s *Server)create(w http.ResponseWriter,r *http.Request){var e store.Delivery;json.NewDecoder(r.Body).Decode(&e);if e.Recipient==""{we(w,400,"recipient required");return};s.db.Create(&e);wj(w,201,s.db.Get(e.ID))}
